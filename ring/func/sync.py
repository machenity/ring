...
class CacheUserInterface(fbase.BaseUserInterface):
    ...
    def get(self, wire, **kwargs):
        ...
        try:
            result = wire._rope.storage.get(key)
    ...
    def update(self, wire, **kwargs):
        ...
        wire._rope.storage.set(key, result)
        return result
...
class BulkInterfaceMixin(fbase.AbstractBulkUserInterfaceMixin):
    ...
    def get_many(self, wire, *args_list):
        ...
        results = wire._rope.storage.get_many(
            keys, miss_value=wire._rope.miss_value)
        return results
    ...
    def update_many(self, wire, *args_list):
        ...
        wire._rope.storage.set_many(keys, values)
        return values
    ...
    def get_or_update_many(self, wire, *args_list):
        keys = self.key_many(wire, *args_list)
        miss_value = object()
        results = wire._rope.storage.get_many(keys, miss_value=miss_value)
        ...
        wire._rope.storage.set_many(new_keys, new_results)
    ...
    def set_many(self, wire, args_list, value_list):
        keys = self.key_many(wire, *args_list)
        wire._rope.storage.set_many(keys, value_list)

    @fbase.interface_attrs(return_annotation=None)
    def delete_many(self, wire, *args_list):
        keys = self.key_many(wire, *args_list)
        wire._rope.storage.delete_many(keys)

    @fbase.interface_attrs(return_annotation=None)
    def has_many(self, wire, *args_list):
        keys = self.key_many(wire, *args_list)
        wire._rope.storage.has_many(keys)

    @fbase.interface_attrs(return_annotation=None)
    def touch_many(self, wire, *args_list):
        keys = self.key_many(wire, *args_list)
        wire._rope.storage.touch_many(keys)
